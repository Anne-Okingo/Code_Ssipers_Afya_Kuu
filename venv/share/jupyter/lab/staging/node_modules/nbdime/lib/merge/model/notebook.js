...
'use strict';
import { arraysEqual, valueIn, hasEntries, stableSort, } from '../../common/util';
import { MergeDecision, resolveCommonPaths, filterDecisions, decisionSortKey, } from '../../merge/decisions';
...
export class NotebookMergeModel {
    ...
    buildCellList(decisions) {
        ...
        for (let cellInfo of cellDecisions) {
            cells.push(new CellMergeModel(cellInfo.base, cellInfo.decisions, this.mimetype));
        }
        return cells;
    }
}
        ...
        if (arraysEqual(md.absolutePath, ['cells'])) {
            if (md.localDiff && !md.remoteDiff) {
                ...
                if (isChunk(md.remoteDiff)) {
                    ...
                    output.push(nmd);
                    output.push(new MergeDecision(md.absolutePath.slice(), md.localDiff.slice(1), md.remoteDiff.slice(1), 'either'));
                }
                else {
                    // Split off local
                    let nmd = new MergeDecision(md);
                    nmd.localDiff = md.localDiff.slice(0, 1);
                    output.push(nmd);
                    output.push(new MergeDecision(md.absolutePath.slice(), md.localDiff.slice(1), [], 'local', // Check for custom action first?
                    md.conflict));
                }
            }
                ...
                nmd.remoteDiff = md.remoteDiff.slice(0, 1);
                output.push(nmd);
                output.push(new MergeDecision(md.absolutePath.slice(), [], md.remoteDiff.slice(1), 'remote', // Check for custom action first?
                md.conflict));
            }
...
function splitCellListPatch(mergeDecisions) {
    ...
    for (let md of mergeDecisions) {
        ...
        for (let i = 0; i < maxlen; ++i) {
            ...
            output.push(new MergeDecision(md.absolutePath.slice(), subdl, subdr, md.action, md.conflict));
        }
    }
...
function splitCellRemovals(mergeDecisions) {
    ...
    let makeSplitPart = function (md, key, local, remote) {
        let newMd = new MergeDecision(md.absolutePath.slice(), null, null, md.action, md.conflict);
        let newDiff = [
            {
                key: key,
                op: 'removerange',
...
function splitCellInsertions(mergeDecisions) {
    ...
    let makeSplitPart = function (md, value, local, remote) {
        let newMd = new MergeDecision(md.absolutePath.slice(), null, null, md.action, md.conflict);
        if ((local && !hasEntries(md.localDiff)) ||
